...
3. **El pipeline de modelado:** filtro → anti‑leakage → `_missing` → log1p → **split estratificado** → winsorización → **KNN** → **SMOTEENN** (todo fiteado solo en train) → **Random Forest / Gradient Boosting**.
4. **Resultado:** accuracy ≈ 0,98, F1 ≈ 0,98, sin overfitting; features top = `sy_dist`, brillo y los `_missing`.
